...
> **下次觸發**：佇列現只有需人工項目，應依 playbook §6 執行完整健檢（三項自我測試 + iconv 掃描 + 資產盤點 + Issue 核對），確認無新問題後維持「等待方向指派」狀態，**不做 commit**（除非使用者已於期間指派新方向或發現新的實質缺陷）。已連續多次健檢結果完全相同，**不需要每次都發 push notification**——僅在數據出現變化（新 Issue、CI 失敗、資產計數變動等）或使用者尚未被提醒過時才發送，避免重複打擾。
