...
async def run_real_diagnose(turbine_id: str = "Kelmarsh_1") -> dict:
    ...
    # ── Step 1: 專案總監分派 ──
    await _update_agent(
        "project-director", AgentStatus.WORKING, f"確認 {turbine_id} 風機資訊", 0.0
    )
    await _broadcast_log("project-director", "專案總監", f"🚀 啟動 {turbine_id} 故障診斷工作流程")
    await asyncio.sleep(1)
    await _broadcast_log(
        "project-director", "專案總監", "Kelmarsh 風場 — Senvion MM92, 2050 kW, 92m 轉子直徑"
    )
    await _update_agent(
        "project-director", AgentStatus.WORKING, f"確認 {turbine_id} 風機資訊", 0.5
    )
    await asyncio.sleep(1)
    await _update_agent(
        "project-director", AgentStatus.COMPLETED, f"已確認 {turbine_id} 風機資訊", 1.0
    )
    await _broadcast_log("project-director", "專案總監", "任務分派完成，開始診斷流程", "success")

    # ── Step 2: 載入真實資料 ──
    await _update_agent(
        "fault-diagnostician", AgentStatus.WORKING, "載入 SCADA 資料", 0.0, ["predictive-modeler"]
    )
    await _update_agent(
        "predictive-modeler", AgentStatus.WORKING, "載入 SCADA 資料", 0.0, ["fault-diagnostician"]
    )
    await _broadcast_log(
        "fault-diagnostician", "故障診斷師", f"正在載入 {data_id} 的 SCADA 資料..."
    )
    ...
    try:
        ...
        await _broadcast_log(
            "fault-diagnostician", "故障診斷師", f"資料載入成功：{len(df)} 筆記錄", "success"
        )

    except Exception as e:
        await _broadcast_log(
            "fault-diagnostician", "故障診斷師", f"資料載入失敗：{str(e)}", "error"
        )
    ...
    try:
        ...
        await _broadcast_log(
            ...
            f"移除異常值: {quality_report['outliers_removed']} 筆",
            "success",
        )
        ...
        await _broadcast_log(
            "fault-diagnostician", "故障診斷師", f"清洗過程警告：{str(e)}", "warning"
        )
        df_clean = df
    ...
    await _broadcast_log(
        "predictive-modeler", "預測模型師", "資料前處理完成，移交診斷分析", "success"
    )
    ...
    try:
        ...
        await _broadcast_log(
            "fault-diagnostician", "故障診斷師", f"特徵工程部分失敗：{str(e)}", "warning"
        )
        df_feat = df_clean
    ...
    try:
        ...
        report = await loop.run_in_executor(
            None, lambda: generate_diagnosis_report(df_feat, data_id)
        )
        ...
        if ops:
            ...
            await _broadcast_log(
                "fault-diagnostician", "故障診斷師", f"容量因數：{cf:.1f}%, 可用率：{avail:.1f}%"
            )
    ...
    await _broadcast_log(
        "literature-reviewer", "文獻審閱員", "找到 3 篇相關文獻，整理維護建議", "success"
    )
